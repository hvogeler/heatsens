                              ...
                              int32_t event_id, void *event_data)
{
    ...
    {
        if (wifi.s_retry_num_ < WIFI_MAXIMUM_RETRY)
        {
            esp_wifi_connect();
            wifi.s_retry_num_++;
    ...
    else if (event_base == IP_EVENT && event_id == IP_EVENT_STA_GOT_IP)
    {
        ...
        wifi.s_retry_num_ = 0;
        xEventGroupSetBits(wifi.s_wifi_event_group_, WIFI_CONNECTED_BIT);
        wifi.is_connected = true;
    }
}

esp_err_t Wifi::wifi_connect(void)
{
    esp_err_t ret = ESP_OK;
    Wifi::is_connected = false;

    // Initialize event loop only once
    // Calling this multiple times will cause ESP_ERR_INVALID_STATE
    ...
    Nvs nvs_creds;
    ret = nvs_creds.open_namespace("config");
    if (ret == ESP_OK)
    {
    ...
    {
        ESP_LOGI(TAG, "connected to ap SSID:%s", wifi_ssid.c_str());
        return ESP_OK;
    }
    else if (bits & WIFI_FAIL_BIT)
    {
        ESP_LOGI(TAG, "Failed to connect to SSID:%s", wifi_ssid.c_str());
        return ESP_FAIL;
    }
    else
    {
...
esp_err_t Wifi::time_sync(void)
{
    setenv("TZ", CONFIG_HEATSENS_TIMEZONE, 1);
    tzset();

    esp_sntp_setoperatingmode(SNTP_OPMODE_POLL);
    esp_sntp_setservername(0, CONFIG_HEATSENS_TIME_SERVER_0);
    esp_sntp_setservername(1, CONFIG_HEATSENS_TIME_SERVER_1);
    esp_sntp_init();
