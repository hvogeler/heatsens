...
esp_err_t Bmp280::read_raw(int32_t *raw_temp, int32_t *raw_press)
{
    ...
    esp_err_t err = i2c.transmit(dev_handle, BMP280_REG_CTRL_MEAS, 0x56); // osrs_t=010, osrs_p=101, mode=10
    if (err != ESP_OK)
    {
...
esp_err_t Bmp280::init()
{
    ...
    if (!found)
    {
        ESP_LOGE(TAG, "BMP280/BME280 not found at either 0x76 or 0x77");
        return ESP_FAIL;
    }
    ...
    err = i2c.transmit(dev_handle, BMP280_REG_CTRL_MEAS, 0x56); // osrs_t=010, osrs_p=101, mode=10
    if (err != ESP_OK)
    {
    ...
    err = i2c.transmit(dev_handle, BMP280_REG_CONFIG, 0x00);
    if (err != ESP_OK)
    {
